...
contract DCX918Market {
    ...
    modifier OnlyItemOwner(address tokenAddress, uint256 tokenId){
     ...
     modifier ItemExists(uint256 id){
        require(id < itemsForSale.length && itemsForSale[id].id == id, "Can find nothin");
        _;
    }

    modifier IsForSale(uint256 id){
        require(itemsForSale[id].isSold == false, "Somebody already bought this!");
        _;
    }
    ...
    function addItemToMarket(uint256 tokenId, address tokenAddress, uint256 askingPrice) OnlyItemOwner(tokenAddress,tokenId) HasTransferApproval(tokenAddress,tokenId) external returns (uint256){
        require(activeItems[tokenAddress][tokenId] == false, "Its already up for sale!");
